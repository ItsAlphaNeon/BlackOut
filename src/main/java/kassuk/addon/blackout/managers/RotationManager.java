            ...
            if (updateShouldRotate()) {
                ...
                if (rotated) {
                    mc.getNetworkHandler().sendPacket(
                            new PlayerMoveC2SPacket.LookAndOnGround(next[0], next[1], Managers.ON_GROUND.isOnGround()));
                }
            }
        }
    }
    ...
    private void onRender(Render3DEvent event) {
        ...
        if (timer > 0 && target != null && lastDir != null) {
            if (SettingUtils.shouldVanillaRotate()) {
                mc.player.setYaw(MathHelper.lerpAngleDegrees(mc.getRenderTickCounter().getTickDelta(true), prevDir[0],
                        currentDir[0]));
                mc.player.setPitch(
                        MathHelper.lerp(mc.getRenderTickCounter().getTickDelta(true), prevDir[1], currentDir[1]));
            }
        } else if (target != null) {
            target = null;
            priority = 1000;
